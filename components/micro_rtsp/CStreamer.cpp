...
#define KJpegHeaderSize 8           // size of the special JPEG payload header
    ...
    /* ESP32 uses RTP-over-TCP only; larger fragments avoid ~20 blocking sends per frame. */
    int max_fragment = 1100;
    LinkedListElement *frag_element = m_Clients.m_Next;
    while (frag_element != &m_Clients) {
        CRtspSession *frag_session = static_cast<CRtspSession *>(frag_element);
        if (frag_session->isTcpTransport()) {
            max_fragment = 8192;
            break;
        }
        frag_element = frag_element->m_Next;
    }
    int fragmentLen = max_fragment;
    ...
    static char RtpBuf[16384]; // sized for 8 KiB TCP fragments + RTP/JPEG headers
